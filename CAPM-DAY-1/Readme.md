...
the CAP framework is used of building enterprise grade applications and extension which are cloud native in SAP BTP.
</p>
</br>
</br>
...
<p>
Conceptually CDS is same concept in ABAP but with different flavor in CAPM, The concept says that , we have a design time object which is a cds file, we will create a *.cds file for almost everything eg. database tables, view, model, services, Ui/UX
</br>
</br>

  - [x] This design time object file is compiled by CAPM framework (inside CAP we have node module called <b>@sap/cds</b>)
  - [x] Once the compilation is successful a <b>runtime</b> object gets created
...
- [x] install node version manager nvm to manage node version in system https://github.com/coreybutler/nvm-windows/releases/tag/1.1.12
</br>
if there is multiple node projects which is associated with different version user will be faced with a challenge of switching
...
- [x] When CDS initialization in Vs code or BAS is successful you may see following objects generated in your development folder 
</br>
</br>
...
- [x] create 2 files with same name with different format (the file names should be identical  it is mandatory for CAP functionality to work)
...
Execute the CDS file in terminal 
...
the result will look like this (to change the version mentioned in highlighted section)  check the <b>package.json</b> file 
</br>
</br>
...
<p>
  A namespace define a unique name for our group. It also give uniqueness to the project.
  for Example : in my company we have 2 cap project, one for finance and other for HR.
...
(SQlite a lite weight db used in cap development for local development and testing purpose its a open source file system based DB) 
</br>
</br>
...
Vs code may automatically suggest appropriate tool when you try to access the DB connection (it could be SQite 3 for node) and also prompts to reload the window
</br>
</br>
...
- [x] create a CSV file in the name of the database table with different naming convention
...
check the records in the db as mentioned below
</br>
</br>
